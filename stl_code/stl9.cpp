...
int main(){
    ...
    for(auto& height : height_ins){//变量名和上一个for循环一样
        height = h;
        h += ht_step;
    }

    std::cout << std::setw(7) << "|";//std::setw(x)填充x个字符
    for(const auto& w : weight_lbs)//auto 加了const不修改:后面变量
    ...
    for(const auto& h : height_ins){
        ...
        std::cout << std::fixed << std::setprecision(1);//浮点数,精度为1
